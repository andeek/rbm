(function(){
  var Surface=function(node){
    ...
    var data = [$.map(node.datum(), function(value, index) {  return [value]; })];
    ...
    var getTransformedData=function(){
      ...
      var ylength=data[0][0].length;
      
      for(var x=0;x<xlength;x++){
        output.push(t=[]);
        for(var y=0;y<ylength;y++){
          t.push(transformPoint([(x-xlength/2)/(xlength*1.41)*displayWidth*zoom, heights[0][x][y]*zoom, (y-ylength/2)/(ylength*1.41)*displayWidth*zoom]));
        }
      }
    ...
    var renderSurface=function(){
      var newdata=getTransformedData();
      var xlength=newdata.length;
      var ylength=newdata[0].length;
      var d0=[];
      var idx=0;
      for(var x=0;x<xlength-1;x++){
        for(var y=0;y<ylength-1;y++){
          var depth=newdata[x][y][2]+newdata[x+1][y][2]+newdata[x+1][y+1][2]+newdata[x][y+1][2];
          d0.push({
            path:
              'M'+(newdata[x][y][0]+displayWidth/2).toFixed(10)+','+(newdata[x][y][1]+displayHeight/2).toFixed(10)+
              'L'+(newdata[x+1][y][0]+displayWidth/2).toFixed(10)+','+(newdata[x+1][y][1]+displayHeight/2).toFixed(10)+
              'L'+(newdata[x+1][y+1][0]+displayWidth/2).toFixed(10)+','+(newdata[x+1][y+1][1]+displayHeight/2).toFixed(10)+
              'L'+(newdata[x][y+1][0]+displayWidth/2).toFixed(10)+','+(newdata[x][y+1][1]+displayHeight/2).toFixed(10)+'Z',
            depth: depth, newdata: data[0][x][y]
          });
        }
      }
      ...
      if(colorFunction){
        dr.attr("fill",function(d){return colorFunction(d.newdata)});
      }
      trans=false;
    };
