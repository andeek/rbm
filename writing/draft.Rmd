---
title: "Properties and Bayes fitting of restricted Boltzmann machines"
author: 
  - name: Andee Kaplan
...
\end{equation} 
for the outcomes $(\pm 1, \pm 1)$,  values for parameters $(\theta_{v_1}, \theta_{v_2}, \theta_{h_1}, \theta_{11},\theta_{21})$ may be chosen to approximate such  cell probabilities with arbitrary closeness.  In fact, when the cell probabilities (\ref{eqn:0}) are all strictly positive, parameters in the RBM model can be specified to reflect these probabilities exactly. And, when one or more of the cell probabilities (\ref{eqn:0}) are zero, the corresponding RBM  probabilities $P(V_1=v_1,V_2= v_2| \theta_{v_1}, \theta_{v_2}, \theta_{h_1}, \theta_{11},\theta_{21})$ may never be identically zero (due to exponential terms in the model) but parameters can be still selected to make the appropriate RBM cell probabilities arbitrarily small.
