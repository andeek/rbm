...
```{r instab, fig.cap="\\label{fig:instab}$(\\max\\limits_{\\boldsymbol x_N \\in \\mathcal{X}_N}[Q(\\boldsymbol x_N)] - \\min\\limits_{\\boldsymbol x_N \\in \\mathcal{X}_N}[Q(\\boldsymbol x_N)])/N$ for various values of $\\boldsymbol \\theta$ for the tiny example model. This quantity is increasing for larger positive values of $\\boldsymbol \\theta$.", fig.height = 4, fig.align='center'}
expand.grid(theta_h1 = seq(-5, 5, length.out = 5),
            theta_v1 = seq(-5, 5, length.out = 5),
            theta_11 = seq(-5, 5, length.out = 31)) %>%
  data.frame() %>%
  group_by(theta_v1, theta_h1, theta_11) %>%
  do(data.frame(q_max = max(possibles %*% t(data.matrix(.))) - min(possibles %*% t(data.matrix(.))))) %>%
  ungroup() %>%
  mutate(theta_h1 = factor(theta_h1, labels = paste0("theta[h1]==", unique(theta_h1))), 
         theta_v1 = factor(theta_v1, labels = paste0("theta[v1]==", unique(theta_v1)))) %>%
  ggplot() +
  ...
  facet_grid(theta_h1 ~ theta_v1, labeller = label_parsed) +
  xlab(expression(theta[11])) +
  ylab(expression(frac(max(Q) - min(Q), H + V)))
```
