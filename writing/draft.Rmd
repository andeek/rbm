...
The RBM class of models can be thought to exhibit an analogous occurence of *near-degeneracy* when random variables in the neg-potential function $Q(\cdot)$ have a mean vector $\mu(\boldsymbol \theta)$ close to the boundary of the convex hull of $\mathcal{S}$, where $t(\boldsymbol x) = \{v_1, \dots, v_V, h_1, \dots, h_H, v_1 h_1, \dots, v_V h_H \} \in \mathcal{S}$ and the mean parameterization on the model parameters, $\mu(\boldsymbol \theta)$ is
\begin{align*}
