...
Within the context of the RBM, this definition concerns the joint distribution of both hidden and visible nodes, where $N$ is the number of nodes in the model, $H + V$. However, instability in terms of only the visible nodes is a more useful measure of model impropriety to be concerned with. To see this, consider the example of classifying images using an RBM. One would be interested in the maximum change in the likelihood when a single pixel is changed in the visibles as a proxy for instability in the model. Thus, we can instead concern ourselves with the notion of instability that deals with the maximum likelihood ratio of two images that differ in only one pixel. 
...
Let $R(\boldsymbol \theta) \equiv \max_{\boldsymbol v} \max_{\boldsymbol h}Q(\boldsymbol x) - \min_{\boldsymbol v}\max_{\boldsymbol h}Q(\boldsymbol x) - H\log 2$. Then, if $R(\boldsymbol \theta) > C*V$, this implies that the maximum log-likelihood ratio of two images that differ in only one pixel is greater than $C$. So, by examining $R(\boldsymbol\theta)/V$, we can determine a lower bound for the maximum log-likelihood ratio of two images that differ in only one pixel, which if large is consistent with the definition of instability detailed above.
