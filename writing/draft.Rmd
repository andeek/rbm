...
@schweinberger2011instability introduces a related concept, *instability*, which means [**TODO: that small changes in the natural parameters result in large changes in probability masses**]. This is characterized by excessive sensitivity in the model, where a small change in $\boldsymbol x$ leads to a large change in the probability mass function. @schweinberger2011instability provides the following formal definition for instability in a discrete exponential family model where [**TODO: notation goes here**].

\begin{defn}
A discrete exponential family distribution is \emph{unstable} if for any $C > 0$, however large, there exists $N_C > 0$ such that 
\begin{align}
\max\limits_{\boldsymbol x_N \in \mathcal{X}_N}[Q(\boldsymbol x_N)] - \min\limits_{\boldsymbol x_N \in \mathcal{X}_N}[Q(\boldsymbol x_N)] > CN \text{ for all } N > N_C \label{eq:unstable}
\end{align}

Conversely, a discrete exponential family distribution is \emph{stable} if there exists constants $C > 0$ and $N_C > 0$ such that 
\begin{align}
\max\limits_{\boldsymbol x_N \in \mathcal{X}_N}[Q(\boldsymbol x_N)] - \min\limits_{\boldsymbol x_N \in \mathcal{X}_N}[Q(\boldsymbol x_N)] \le CN \text{ for all } N > N_C. \label{eq:stable}
\end{align}
\end{defn}

For RBMs, an alaogous concept could concern the joint distribution of both hidden and visible nodes, where $N$ is the number of nodes in the model, $H + V$. However, instability in terms of only the visible nodes is a more useful measure of model impropriety. To see this, consider modeling images using an RBM. One would be interested in the maximum change in the likelihood when a single pixel is changed in the visibles as a proxy for instability in the model. So we concern ourselves with a notion of instability that deals with the maximum likelihood ratio of two vectors of visibless that differ at only one coordinate.
...
Instability in a model is in itself a measure of model impropriety, however there is also a relationship between instability and near-degeneracy in a discrete exponental model.
...
Thus, if a discrete exponential family model is unstable, then it is also degenerate.
