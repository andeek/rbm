...
sample.params %>% 
  ...
  mutate(Value = paste0("$", round(Value, 10), "$")) -> tbl
options("scipen" = old_pen)

cbind(tbl[1:8,], tbl[9:16,], tbl[17:24,]) %>%
  kable(caption = "\\label{tab:theta}Parameters used to fit a test case with $V = H = 4$. This parameter vector was chosen as a sampled value of $\\boldsymbol \\theta$ that was not near the convex hull of the sufficient statistics for a grid point in Figure \\ref{fig:degen_plots} with $< 5$\\% near-degeneracy.", align = rep(c("l", "r"), times = 3))
```
