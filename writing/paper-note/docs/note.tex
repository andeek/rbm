...
Durham, NC 27708-0251, USA \\ \(^*\){\email{Corresponding author: \href{mailto:andrea.kaplan@duke.edu}{\nolinkurl{andrea.kaplan@duke.edu}}}}
...
\((\boldsymbol X, \boldsymbol H)\) from \eqref{eq:RBM1} as \begin{align*}
...
\end{align*} written as a function \begin{equation}
\label{eq:f}
...
\end{equation} of outcomes
\(\boldsymbol x =(x_1,\ldots,x_N) \in\{\pm1\}^{N}\),
...
L1 norm of a generic vector \(\boldsymbol y =(y_1,\ldots,y_d)\),
...
\BeginKnitrBlock{remark}
\iffalse{} {Remark. } \fi{}The condition
...
builds upon and extends related findings by
\citet{schweinberger2011instability}, who largely focused on the case of
...
\end{align*} as \(N \rightarrow \infty\) by the definition of an
S-unstable FOES model \eqref{eq:Sun}. Consequently,
