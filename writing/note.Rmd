...
$$ 
is the normalizing function. Let $\boldsymbol \theta_N = (\boldsymbol \alpha, \boldsymbol \beta,\Gamma) \in \Theta_N \subset \mathbb{R}^{q(N)}$ with $q(N) = N + N_H + N*N_H$ denote the vector of parameters for the RBM.
...
\begin{align}
\frac{1}{N} \log \left[\frac{\max\limits_{(x_1, \dots, x_N) \in \mathcal{X}^N}P_{\boldsymbol \theta_N}(x_1, \dots, x_N)}{\min\limits_{(x_1, \dots, x_N) \in \mathcal{X}^N}P_{\boldsymbol \theta_N}(x_1, \dots, x_N)}\right] \equiv \frac{1}{N} \text{ELPR}_N(\boldsymbol \theta_N). \label{eqn:elpr}
\end{align}
...
\begin{prop}
\label{prop:instab}
Let $\text{ELPR}(\boldsymbol \theta_N)$ be as in (\ref{eqn:elpr}) for an integer $N \ge 1$. For a given $C>0$, if $$\frac{1}{N}\text{ELPR}_N(\boldsymbol \theta_N) > C,$$ 
then 
