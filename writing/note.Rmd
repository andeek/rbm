...
# Discrete Expanding Finite space models and notions of instability
...
\begin{align}
\frac{1}{N} \log \left[\frac{\max\limits_{(x_1, \dots, x_N) \in \mathcal{X}^N}P_{\boldsymbol \theta_N}(x_1, \dots, x_N)}{\min\limits_{(x_1, \dots, x_N) \in \mathcal{X}^N}P_{\boldsymbol \theta}(x_1, \dots, x_N)}\right] \equiv \frac{1}{V} \text{ELPR}_N(\boldsymbol \theta_N) \label{eqn:elpr}
\end{align}
...
Note this somewhat general result can be applied to many model families. In Section \ref{examples-of-def-models}, we present two specific examples of DEF models, including models with deep architectures.

# Examples of DEF models
