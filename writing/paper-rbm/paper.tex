...
\title{Properties and Bayesian fitting of restricted Boltzmann machines}
\authormark{KAPLAN \textsc{et al}}

\author[1]{Andee Kaplan*}
\author[2]{Daniel Nordman}
\author[2,3]{Stephen Vardeman}

\address[1]{
\orgdiv{Department of Statistical Science}, \orgname{Duke University}, \orgaddress{\state{North Carolina}, \country{USA}}}
\address[2]{
\orgdiv{Department of Statistics}, \orgname{Iowa State University}, \orgaddress{\state{Iowa}, \country{USA}}}
\address[3]{
\orgdiv{Department of Industrial and Manufacturing Systems Engineering}, \orgname{Iowa State University}, \orgaddress{\state{Iowa}, \country{USA}}}

\corres{*Andee Kaplan, Department of Statistical Science, Duke University, P.O. Box 90251,
Durham, NC 27708-0251. \email{\href{mailto:andrea.kaplan@duke.edu}{\nolinkurl{andrea.kaplan@duke.edu}}}}
...
RBMs thereby are thought to have the ability to encode very complex and
rich structures in data, making them attractive for supervised learning.
However, the generative behavior of RBMs largely is unexplored and
...
Gibbs sampling (quasi-Bayes) methods often are advocated for the RBM
model structure.
}
...
\jnlcitation{\cname{
\author{Kaplan A.,} 
\author{D. Nordman, and} 
\author{S. Vardeman} 
 (\cyear{2018+}), 
\ctitle{Properties and Bayesian fitting of restricted Boltzmann machines}, \cjournal{Stat Anal DataMin: The ASA Data Sci Journal}, \cvol{2018+;00:00-00}.}}
...
The data mining and machine learning communities recently have shown
...
examples). An RBM is a probabilistic undirected graphical model (for
...
1986). These models reportedly have been used with success in
...
Box 1967). At issue, simulation from RBMs often can produce data lacking
...
statistical concern in that, realistically, many data processes of
interest are not degenerate in their spectrum of potential outcomes. For
example, when sampling data from a nearly degenerate RBM used to model
an imaging process, only a small set of output possibilities receives
nearly all probability, and thus a sample of images will all be copies
of the same one image (or small number of images). An example of ten
4-pixel images simulated from a nearly degenerate RBM model is compared
to ten 4-pixel images simulated from a non-degenerate RBM model in
Figure \ref{fig:sample-models}. The degenerate model places almost all
...
In addition to such degeneracy, we find that RBMs easily can exhibit
...
of such behavior recently have been documented in RBMs (Li 2014), as
...
latent variables (hiddens), which can critically influence convergence
in MCMC-based likelihood methods. Applying basic statistical principles
in fitting RBM models of tractable size, we compare three fully Bayesian
techniques involving MCMC, which are computationally more accessible
than direct maximum likelihood and also aim to avoid parts of an RBM
...
fitting traceable to model flexibility potentially are more concerning.
For an RBM model with enough hidden variables, any distribution for the
...
parametric model class even is introduced and applied to obtain a
refinement of model fit. As a consequence, we find that any fully
principled fitting method based on the likelihood for an RBM with enough
...
``smoothed distribution'' may result when fitting an RBM model of
sufficient size with a rigorous likelihood-based method. We therefore
are led to be skeptical that models that involve these structures (like
deep Boltzmann machines) can achieve useful prediction or inference in a
...
distributions. These are tied closely to the probability properties of
...
most commonly being considered. In this paper, we consider the binary
case for concreteness. An RBM architecture has two layers, hidden
...
treats the pixel values as the visible variables \(v_i\) in an RBM model
...
the states of the visible and hidden nodes in an RBM for some integers
...
this section we define these, consider how to quantify them in an RBM,
...
\emph{not} be pulled deeply into the interior of \(C\).

An RBM model can be thought to exhibit an analogous form of
...
Instability roughly can be thought of as excessive sensitivity in the
...
in probability structure, with model degeneracy included as an extreme
or limiting case of instability. To quantify ``instability'' more
...
in this process one also may expand arbitrarily the number of hidden
...
In formulating an RBM model for a potentially large number of visibles
...
In other words, an RBM model sequence is unstable if, given any
...
\] denoting the largest log-probability ratio for a one-component change
in data outcomes (visibles) at an RBM parameter
...
then an RBM model sequence will exhibit large probability shifts for
very small changes in data configurations (i.e., will exhibit
instability). Such instability can be a concern in a model for reasons
similar to those related to degeneracy: as outcomes vary over the sample
space, the geography of probabilities is extremely rugged, with deep
pits following sharp mountains. Recall the applied example of RBM models
as a means to classify images. For data as pixels in an image, the
instability result in Proposition \ref{prp:instab1} manifests itself as
a one pixel change in an image (one component of the visible vector),
resulting in a large shift in the probability, which in turn could
result in a vastly different classification of the image. Examples of
this kind of behavior have been presented in Szegedy et al. (2013) for
deep learning models, in which a one pixel change in a test image
results in a wildly different classification.

Additionally, S-unstable RBM model sequences may be connected formally
...
\] for a given \(0 < \epsilon < 1\). Then, S-unstable model sequences
are guaranteed to be degenerate, as the following result shows.
...
potentially could be arbitrarily narrow. Proofs of Propositions
...
statement. We adapt this notion to RBM models. Note that in an RBM, the
parameters \(\theta_{v_1}, \dots, \theta_{v_\nv}\) and
\(\theta_{h_1}, \dots, \theta_{h_\nh}\) naturally are associated with
...
and \(\theta_{h_j}\) no longer are interpretable in the models
...
non-degenerate models, while degradation in interpretability often is
...
Uninterpretability then is avoided at a parametric specification
...
uninterpretability in an RBM, we consider first the smallest possible
...
three dimensions, we are able to illustrate explicitly the shape of
...
\caption{\label{fig:degen-toy}Contour plots for the three parametric mean functions of sufficient statistics for an RBM with one visible and one hidden node.}\label{fig:degen-toy}
\end{figure}
...
model uninterpretability, we can look at the difference between
E\(\left[\boldsymbol X | \boldsymbol \theta\right]\) model expectations
and E\(\left[\boldsymbol X | \boldsymbol \theta^*\right ]\) expectations
given independence for the tiny toy RBM model where
\(\boldsymbol X = (V_1, H_1, V_1 H_1)\). This difference is given by
\begin{align*}
\text{E}\left[\boldsymbol X | \boldsymbol \theta\right] -   \text{E}\left[\boldsymbol X | \boldsymbol \theta^* \right] 
&= \left[
\begin{matrix}
...
coordinates of the vector of
E\(\left[\boldsymbol X | \boldsymbol \theta\right]\) model expectations
and E\(\left[\boldsymbol X | \boldsymbol \theta^*\right ]\)
corresponding expectations given independence grow for the toy RBM as
the values of \(\boldsymbol \theta\) are farther from zero, especially
for large magnitudes of \(\theta_{11}\), the dependence parameter. This
is a third indication that parameter vectors of large magnitude readily
...
this and further relations between the \(-1/1\) data encoding and the
\(\boldsymbol \theta\) parameterization of the RBM model class are given
in the on-line supplementary materials. Based on this material,
Proposition \ref{prp:three} also briefly summarizes how the \(-1/1\)
data encoding induces some attractive theoretical properties in
characterizing RBM models.
...
Hence, while from some computing perspectives \(0/1\) coding might seem
most natural, the \(-1/1\) coding is far more convenient and
interpretable from the point of view of statistical modeling, where it
makes parameters simply interpreted in terms of symmetrically defined
main effects and interactions. Under the data encoding \(-1/1\), the
parameter space centered at 0 is also helpful for framing parameter
configurations that are undesirably large (i.e., these are naturally
parameters that have moved too far away from \(\bm{0}\) where the RBM
model is anchored to be trivially describable and completely
problem-free). In light of all of these matters we will henceforth
employ the \(-1/1\) coding.
...
average magnitudes, we later are able to consider the potential benefit
...
vectors then are paired to create 100 values of \(\boldsymbol \theta\)
...
\begin{figure}
\centering
\includegraphics{paper_files/figure-latex/degen-plots-1.eps}
...
\begin{figure}
\centering
\includegraphics{paper_files/figure-latex/instab-plots-1.eps}
...
\begin{figure}
\centering
\includegraphics{paper_files/figure-latex/uninterp-plots-1.eps}
...
\begin{figure}
\centering
\includegraphics{paper_files/figure-latex/split-plots-dist-1.eps}
...
number \(\nh\) of hidden variables increases. Additionally, the slope
becomes steeper as \(\nh\) increases as well.}
\end{figure}
...
Typically, fitting an RBM via maximum likelihood (ML) methods will be
unfeasible due mainly to the intractability of the normalizing term
\(\gamma(\boldsymbol \theta)\) in a model \eqref{eq:pmf} of any
realistic size. Ad hoc methods often are recommended instead, which aim
...
However, computational concerns are not the only issues with fitting an
RBM using ML. In addition, an RBM model, with the appropriate choice of
...
approximated arbitrarily well by an RBM with \(2^{\nv-1} - 1\) hidden
units. We provide a small example that illustrates such approximations.

For simplicity, consider a model with two visible variables
\((V_1,V_2)\) and one hidden \(H_1\) so that there are four possible
data realizations for \((V_1,V_2)\) given by \((\pm 1, \pm 1)\), and we
may express the marginalized model probabilities as \[
...
probabilities \eqref{eq:0} are zero, the corresponding marginalized RBM
probabilities
...
but parameters still can be selected to make the appropriate RBM cell
...
2011), but dramatically different parameter settings can induce the same
RBM model (beyond mere symmetries in parameterization). A further (and
somewhat odd) consequence of the RBM parameterization is then that when
fitting the RBM model by likelihood-based methods, we already may know
the nature of the answer before we begin: namely, such fitting simply
will reproduce the empirical distribution of the training data if
sufficiently many hiddens are in the model. There then can be no model
refinements or smoothing from the RBM. That is, based on a random sample
of vectors of visible variables, the model for the cell probabilities
with the highest likelihood over \emph{all possible model classes}
(i.e., RBM-based or not) is the empirical distribution, and the
over-parameterization of the RBM model itself ensures that this
empirical distribution can be arbitrarily well-approximated.

For illustration, considering the simple example from above with \(n\)
iid observations, each consisting of two realized visibles \((V_1,V_2)\)
the empirical cell frequencies from the sample are the highest
likelihood choices for the cell probabilities
...
empty cells, fitting steps for the RBM model further will aim to choose
parameters that necessarily diverge to infinity in magnitude in order to
zero-out the corresponding marginalized RBM cell probabilities. In data
applications with a large sample space, it is unlikely that the training
set will include at least one of each possible vector outcome (unlike
this small example). This implies that some RBM model parameters must
diverge to \(+\infty\) to mimic the empirical distribution with empty
cells and, as we have already discussed in Section
...
both overfitting and model impropriety, testing on an \(\nv = \nh = 4\)
...
\tightlist
\item
  \emph{A ``trick'' prior.} Here we cancel out the normalizing term in
  the likelihood (from \(\gamma(\boldsymbol \theta)\) in \eqref{eq:pmf})
  so that resulting full conditionals of \(\boldsymbol \theta\) are
  ...
  hidden variables \(h_j\) also are treated directly as latent variables
...
\item
  ...
  computation barely is feasible for a problem of this size and would be
  unfeasible for larger problems.) Here the hidden variables \(h_j\)
  again are carried along in the MCMC implementation as latent
  variables. We will refer to this method as Bayes with Truncated Normal
  prior and Latent Variables (BwTNLV).
\item
...
\begin{figure}
\centering
\includegraphics{paper_files/figure-latex/acf-1.eps}
\caption{\label{fig:acf}The autocorrelation functions (ACF) for the
posterior probabilities of all \(2^4 = 16\) possible outcomes for the
...
re-create an empirical distribution perfectly in fitting RBM models
...
\begin{figure}
\centering
\includegraphics{paper_files/figure-latex/fitting-plot-1.eps}
...
used to generate the training data, while the red vertical lines show
the empirical distribution. BwTNML produces the best fit for the data,
...
for Bayesian inference in an RBM model is to use variational methods for
...
avoid areas of the parameter space that potentially will lead to model
...
rigorous likelihood-based fitting for an RBM typically can merely seek
to return the (discrete) empirical distribution for visibles. Current
fitting methodology for RBMs does not provide uncertainty quantification
for the fitted parameters, and it is unclear whether it seeks to address
the issues in model impropriety detailed in this paper. Practitioners
should be aware of this extreme flexibility and employ some form of
regularization or penalization in the fitting.

While interpretability of parameters may not always be of primary
importance in application of an RBM model (e.g.~in some applications to
prediction/classification), we find that the issues of stability and
degeneracy go hand in hand with those of interpretability, and the
former should not be ignored. When S-instability is present, small
(imperceptible) differences in training data and corresponding fitted
models may lead to greatly different probabilities (and thus
substantially different classifications). Ultimately, it is not clear
that RBM models are typically useful as generative models for
observables. Without appropriate generative behavior in an RBM model,
uncertainty in estimated model parameters becomes impossible to
realistically assess. Further, the model cannot support other inferences
important in many prediction problems (e.g., realistic predictive
distributions). For these reasons, we are skeptical about RBMs as widely
useful probabilistic constructs, at least in their present incarnation.
...
Instability and Degeneracy of Deep Learning Models.'' \emph{arXiv
Preprint arXiv:1612.01159}.
...
%\bibliography{../resources/latex/refs\_rbm.bib}
